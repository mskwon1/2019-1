    ...
    if options.boost > 0 and currTime != 0:
        if currTime % options.boost == 0:
            ...
            for j in range(numJobs):
                print '-> Boost %d (timeLeft %d)' % (j, job[j]['timeLeft'])    # 주석처리를 해제함
                if job[j]['timeLeft'] > 0:
                    # print '-> FinalBoost %d (timeLeft %d)' % (j, job[j]['timeLeft'])
                    job[j]['currPri']   = hiQueue
                    job[j]['ticksLeft'] = quantum[hiQueue]  # 수정된 라인!! 원래의 allotment[hiQueue]는 잘못 된 것임
            print 'BOOST END: QUEUES look like:', queue  # 주석처리를 해제함
