...
- melt() -> gather()
  ...
  - 네 번째 인수 : 롱포맷으로 변환할 변수 리스트 지정, 연속된 변수 지정시 : 연산자 사용 가능
    - 포함되지 않은 건 식별자로 감
    - 제외할 열을 -앞에 부텽서 제외가능(식별자 변수로 사용됨)
    - 인덱스, 직접 변수 이름 입력도 가능
  - 파이프 연산자 이용가능(%>%)
- dcast() -> spread()
  - 첫 번째 인수 : data, 변환 대상 데이터셋
...
- separate() : 하나의 열을 여러개의 열로 분할
  ...
  - sep 인수 : 분할에 사용할 문자를 나탄는 정규표현식 / 분할 위치를 나타내는 숫자
...
- stack() : 인수로 주어진 데이터프레임을 두 개의 열을 갖는 롱포맷 데이터프레임으로 변환
