...
SELECT CONCAT(e.Fname, " ", e.Minit, " ", e.Lname) AS Ename, d.Relationship, d.Dependent_name
...
SELECT CONCAT(e.Fname, " ", e.Minit, " ", e.Lname) AS Ename, COALESCE(COUNT(d.Relationship), 0) AS NumOfDependents
FROM employee e LEFT JOIN dependent d ON e.Ssn = d.Essn
GROUP BY e.Ssn
...
FROM department d JOIN dept_locations dl USING (Dnumber) JOIN project p ON dl.Dlocation = p.Plocation
