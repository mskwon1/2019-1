...
SELECT e.Fname, e.Minit, e.Lname, e.Salary*1.1 AS mod_sal
...
FROM department d JOIN employee e ON e.Dno = d.Dnumber JOIN works_on w ON e.Ssn = w.Essn JOIN project p ON p.Pnumber = w.Pno
...
WHERE w.Hours >= 10 AND p.Pname LIKE 'ProductX';
...
SELECT e.Fname, e.Minit, e.Lname, COUNT(Relationship) AS NumOfDependents
FROM employee e JOIN dependent d ON e.Ssn = d.Essn
WHERE e.Ssn IN (
					SELECT DISTINCT e.Ssn
					FROM employee e JOIN dependent d ON e.Ssn = d.Essn
                    WHERE d.Relationship IN ('Daughter', 'Son')
				)
GROUP BY e.Ssn;
